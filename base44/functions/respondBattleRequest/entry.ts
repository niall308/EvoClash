...
Deno.serve(async (req) => {
  try {
    ...
    const oppDeckCards = oppDecks[0]
      ? await base44.asServiceRole.entities.Card.filter({ ownerId: fromUser.id, deckId: oppDecks[0].id })
      : [];
    ...
    const oppCards = oppDeckCards.length
      ? oppDeckCards
      : await base44.asServiceRole.entities.Card.filter({ ownerId: fromUser.id });
