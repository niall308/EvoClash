...
export function buildEggHatchRecolorPrompt(
  ...
  const prompt = `Take this exact creature illustration as the source image. The source image has a TRANSPARENT background (any checkerboard pattern you see is just the transparency indicator, NOT part of the image — it must NOT appear in your output). Keep the creature's pose, anatomy, body structure, face, and art style IDENTICAL to the provided image — do not redesign or restructure the creature. Recolor the creature's body (skin, scales, fur, feathers, hide) using ${palette} applied smoothly across the creature while preserving all shading, highlights, and texture detail. REPLACE the fully transparent original background with a fitting, FULLY OPAQUE, completely rendered ${cardData.type} elemental background as described below — do NOT keep any transparent or empty areas. ${backgroundInstruction} The background must NOT reuse the creature's color gradient — keep it a separate, atmospheric, magical ${cardData.type} environment that contrasts with the creature so the creature stays the clear focal point. The final image MUST be a fully opaque, solid image with NO transparency, NO checkerboard pattern, NO empty/see-through areas anywhere — every pixel must be painted. Output the recoloured creature on the new fully opaque ${cardData.type} background. No text, no border, no frame.`;
