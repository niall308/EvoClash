...
export function buildEggHatchRecolorPrompt(
  ...
  const prompt = `Take this exact creature illustration as the source image. The creature is photographed on a plain solid background. Keep the creature's pose, anatomy, body structure, face, and art style IDENTICAL to the provided image — do not redesign or restructure the creature. Recolor the creature's body (skin, scales, fur, feathers, hide) using ${palette} applied smoothly across the creature while preserving all shading, highlights, and texture detail. REPLACE the plain solid background with a fitting, FULLY OPAQUE, completely rendered ${cardData.type} elemental background as described below. ${backgroundInstruction} The background must NOT reuse the creature's color gradient — keep it a separate, atmospheric, magical ${cardData.type} environment that contrasts with the creature so the creature stays the clear focal point. The final image MUST be a fully opaque, solid image with every pixel painted — NO transparency, NO checkerboard pattern, NO empty or see-through areas anywhere. Output the recoloured creature on the new fully opaque ${cardData.type} background. No text, no border, no frame.`;
