...
// Reusable player profile modal for any PvP surface that shows a player name.
// Displays username, avatar, rank, win/loss record, and head-to-head vs the
// caller. When a `friend` record is supplied, also shows a Remove Friend action.
export default function PlayerProfileModal({ player, friend, onClose, onRemove }) {
  ...
  useEffect(() => {
    ...
    setError("");
    base44.functions
      .invoke("getPlayerProfile", { playerUserId: player.id })
      .then(({ data }) => {
        if (data?.error) setError(data.error);
        else setStats(data);
      })
      .catch(() => setError("Couldn't load this player's profile."));
  }, [player.id]);

  const handleRemove = async () => {
    if (!friend || !window.confirm(`Remove ${player.name} as a friend?`)) return;
    setRemoving(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        {!stats ? (
          error ? (
            <p className="text-red-400 text-sm my-4 text-center">{error}</p>
          ) : (
          ...
          <>
            <div className="flex flex-col items-center -mt-2 gap-2">
              {stats.profilePictureUrl ? (
                ...
                <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center text-white/50 text-xl font-black">
                  {(stats.username || "P")[0]?.toUpperCase()}
                </div>
              )}
              <h2 className="text-lg font-black text-white">{stats.username}</h2>
            </div>
            ...
            <div className="w-full">
              ...
              {stats.headToHead.totalGames === 0 ? (
                <p className="text-white/40 text-xs">You haven't played this player yet.</p>
              ) : (
                <div className="flex items-center justify-center gap-3 bg-white/5 rounded-xl p-3">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="font-black text-lg text-red-400">{stats.headToHead.opponentWins}</span>
                    <span className="text-[10px] text-white/50">{stats.username}</span>
                  </div>
                </div>
              )}
            </div>
