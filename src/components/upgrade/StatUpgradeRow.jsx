...
export default function StatUpgradeRow({ upgrade, card, coins, purchasing, onPurchase, capValue }) {
  const currentVal = card[upgrade.key] || 0;
  const newVal = Math.min(capValue, Math.round(currentVal * (1 + upgrade.percent / 100)));
  const maxed = newVal <= currentVal;
  ...
  return (
    <div className="bg-white/5 rounded-xl p-4 flex items-center justify-between">
      <div>
        ...
        <p className="text-[10px] text-white/40 mt-1">{maxed ? "Maxed for this tier" : `${currentVal} → ${newVal}`}</p>
      </div>
      <button
